 ...
 * BZ23916          091017    Help Desk Error instead of application response if customer has previously been approved.
 * BZ23907          101017    [GS Credit Application] - The approval notification on screen and printed shopping pass are missing the $
...
public class CawGoodSamVisaPrescreenApplyOp extends AbstractPromptOp {
    ...
    /*Begin BZ23579*/
    private static final Logger   _logger               = LogManager.getLogger(CawGoodSamVisaPrescreenApplyOp.class);
    /*End BZ23579*/
    ...
    public IOpResponse handleOpExec(IXstEvent argArg0) {
        ...
        if (!COMPLETE_PRINT.equals(this.getOpState())) {//BZ24556
            ...
            if (_cawEigenMgr.promptDateOfBirthEntry()) {
                if (_cawEigenMgr.promptSocialSecurityEntryPartial()) {
                    if (_cawEigenMgr.enterAnnualIncome()) {
                        if (_cawEigenMgr.displayPhoneNumbers(custParty)) {
                            // BZ 25263
                            isFulfillAllSteps = Boolean.TRUE;
                            if (_cawEigenMgr.preScreenAcceptanceRequest(custParty, _gsHelper.getPreScreenId())) {
                                /*****************
        ...
        //Customer press cancel on pinpad during pinpad entry
        if (isCustPressCancelOnPinPad == Boolean.TRUE) {//BZ25884
            IFormattable args[] = CawUtilFunction.promptArgs(_ff.getSimpleFormattable("_cancelledByCustomer"));
            ...
            /**
             * Begin BZ27344 - added to use refreshPinpadScreen(_transactionScope)
             * After the transaction have applied the GoodSamVisa, The transaction will apply the discount.
             * Therefore the items need to update new price on Pinpad. 
             */
            ...
            if (isFulfillAllSteps) {
                ...
                String requestCareTaker = _cawGoodSamVisaHelper
                        .getCareTakerTemplate(trans, ebsResponse(custParty), CawEigenConstants.PRESCREEN_ACCEPT, _gsHelper
                                .getPreScreenId());
                /* END BZ29280 */
                /* END BZ28247 */
                /* BEGIN BZ28012 */
