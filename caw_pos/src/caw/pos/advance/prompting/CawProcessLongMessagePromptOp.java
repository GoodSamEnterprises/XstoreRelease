 ...
 * 
 */
public class CawProcessLongMessagePromptOp extends AbstractPromptOp {
            ...
            for (int i = 0; i < length; i++) {
                try {
                    ...
                    if (objMessage.getString(CawEBSConstant.TYPE_ATTR)
                            .equals(CawEBSConstant.CATALYST_LONG_MESSAGES_TYPE)
                            && objMessage
    ...
    protected IFormattable[] getPromptArgs(IXstEvent argEvent) {
        ...
        try {
            objMessage = messages.getJSONObject(0);
    ...
    public IOpResponse handlePromptResponse(IXstEvent argParamIXstEvent) {
        ...
        try {
            ...
            _logger.debug("Can not remove long message from message."
                    + ex.getMessage());
        }
