 ...
 * 
 */
public class CawProcessWarningMessagePromptOp extends AbstractPromptOp {

    private static final Logger _logger  = LogManager
            .getLogger(CawProcessLongMessagePromptOp.class);
            ...
            for (int i = 0; i < length; i++) {
                try {
                    ...
                    if (objMessage.getString(CawEBSConstant.TYPE_ATTR)
                            .equals(CawEBSConstant.CATALYST_WARNING_MESSAGES_TYPE)
                            && objMessage
    ...
    public IOpResponse handlePromptResponse(IXstEvent argParamIXstEvent) {
        ...
        try {
            if (CawCatalystHelper.getCatalystMessageResponse() != null
                    && CawCatalystHelper.getCatalystMessageResponse()
                            .length() > 0) {
                CawCatalystHelper.removeElementOfJsonArray(CawCatalystHelper
                        .getCatalystMessageResponse(), 1);
            }
        } catch (JSONException ex) {
            _logger.debug("Can not process message." + ex.getMessage());
        }
