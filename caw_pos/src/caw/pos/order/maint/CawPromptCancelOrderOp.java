 ...
 *
 */
public class CawPromptCancelOrderOp extends PromptYesNoOp {
    ...
    public IOpResponse handlePromptResponse(IXstEvent argEvent) {
        ...
        if (argEvent instanceof IXstDataAction) {
            ...
            if (PARTIAL_UPDATE.equals(action.getActionKey().toString())) {
                this.setScopedValue(CawValueKeys.IS_PARTIAL_CANCEL_ORDER, true);
    ...
    private boolean hasOrderItemSelection() {
        ...
        if (orderLines == null || orderLines.isEmpty()) {
            List<IOrderLine> itemSelections = this.getScopedValue(ValueKeys.ORDER_LINES_TO_PROCESS);
    ...
    private void initializeOrderItemsToProcess() {
        ...
        if (orderLines == null) {
            orderLines = this.getScopedValue(ValueKeys.ORDER_LINES_TO_PROCESS);
