 ...
 * BZ56115          050423    [PROD] [Loyalty] BTM-295 - Registers Stating "Loyalty service offline" when printing reciept
 ...
 *
 */
public class CawEmailAddressPromptOp extends EmailAddressPromptOp {

    private static final Logger            _logger        = LogManager.getLogger(CawEmailAddressPromptOp.class);

    private CawCheetahHelper               _cheetahHelper = CawCheetahHelper.getInstance();
    
    @Inject
    private OrderMgr                       _orderMgr;/*BZ50112*/

    private static final IXstDataActionKey EMAIL_CANCEL   = XstDataActionKey.valueOf("EMAIL_CANCEL");
    private static final IXstDataActionKey EMAIL_AND_PRINT = XstDataActionKey.valueOf("EMAIL_AND_PRINT");
    ...
    @Inject
    private Provider<AccountManager> _amProvider; //BZ53752, BZ56115
    ...
    protected IOpResponse handleFormResponse(IXstEvent argEvent) {
        key = ((IXstDataAction) argEvent).getActionKey();  //BZ63054
        ...
        if (key != XstDataActionKey.NO && _gsHelper.isEnableLoyalty()) {
            ...
            //END BZ58836, BZ58840
                if (!hasRunEnableLoyalty) {
                    ...
                    if (hasRunEnableLoyalty) {
                    	_transactionScope.setValue(CawValueKeys.CAW_DISABLE_TOKEN_IS_USED, true);
                    	preKey = key;
        ...
        if (key != EMAIL_CANCEL 
            ...
            try {
                ...
                _logger.info("Can't read file : " + ex.getMessage());
                return HELPER.getPromptResponse(PromptKey.valueOf("CAW_LOYALTY_OFFLINE_MESSAGE"));
            }
        }
        //END BZ62219
        IPosTransaction posTrans = this._transactionScope.getTransaction();
        EmailAddressFormModel model = this.getModel();
        model.commitChanges();
        String emailAddress = model.getEmailAddress();
        ReceiptInfo receiptInfo = this.getScopedValue(ValueKeys.RECEIPT_INFO);
        receiptInfo.setEmailReceipt(true);
        this.setScopedValue(ValueKeys.RECEIPT_EMAIL_ADDRESS, emailAddress);
        EmailRcptHelper.setReceiptEmailAddress(posTrans, emailAddress);
        boolean printReceipts;
        if (key == EMAIL_AND_PRINT) {
            printReceipts = true;
            EmailRcptHelper.setReceiptDeliveryMethods(posTrans,
                    new EmailRcptDeliveryType[]{EmailRcptDeliveryType.EMAIL, EmailRcptDeliveryType.PAPER});
        } else {
            printReceipts = false;
            EmailRcptHelper.setReceiptDeliveryMethods(posTrans,
                    new EmailRcptDeliveryType[]{EmailRcptDeliveryType.EMAIL});
        }
        
        _gsmPitchesHelper.setTemporaryMembershipCard(_transactionScope); //BZ61159     
        receiptInfo.setPrintReceipt(printReceipts);
    ...
    private void callEarningsAPI() {
        ...
        if(_transactionScope.getValue(CawValueKeys.CAW_AGIS_PITCHES_JSON) != null
            ...
            String pitchesStr = CawAGISPitchesHelper.getInstance().updatePitchesJson(_transactionScope,cawAGISPitchesModel, _transactionScope.getValue(CawValueKeys.CAW_AGIS_PITCHES_JSON)); //BZ69389
    ...
    private void mappingReceiptType(CawCheetahSubmitRequestModel requestModel) {
       ...
       if (key == EMAIL_AND_PRINT) {
           receipt = CawReceiptTypeMappingEnum.PRINT_EMAIL;
        } else {
            receipt = CawReceiptTypeMappingEnum.EMAIL_ONLY;
        }               
        EmailAddressFormModel model = this.getModel();
        String emailAdress = model.getEmailAddress();
        if(emailAdress != null && !emailAdress.isEmpty()) {
            requestModel.setEmailReceipt(emailAdress);
        }       
