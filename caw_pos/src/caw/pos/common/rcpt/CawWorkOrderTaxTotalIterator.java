...
/**
 * Work order receipt: go through all available tax.
 */
public class CawWorkOrderTaxTotalIterator extends TaxTotalIterator {
    ...
    protected void iterate(IPosDocument argDoc,
        ...
        IPosTransaction trans = null;

        if (argO instanceof ICustomerAccountMaintModel) {
            trans = ((ICustomerAccountMaintModel) argO).getCurrentTransaction();
            for (ISaleReturnLineItem item : trans
            ...
            ITaxLineItem taxLine = this.createTaxExemptLine(trans);
            if (taxLine != null) {
                results.put(taxLine.getTaxGroupId(), taxLine);
            }
            if (results.isEmpty() && (taxLine = this
                    .createTaxExemptLineItem(trans.getTotal())) != null) {
                results.put(taxLine.getTaxGroupId(), taxLine);
            }
        }
        if (!results.isEmpty()) {
    ...
    private ITaxLineItem createTaxExemptLine(IPosTransaction argTrans) {

        BigDecimal exentAmt = BigDecimal.ZERO;
        ITaxLineItem taxLine = null;
        List<IRetailTransactionLineItem> items = argTrans.getSaleLineItems();
