...
set _REALPATH=%~dp0
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\reqenvdeployproc.conf"
set _WRAPPED_APP=reqenvdeployproc
set PLATFORM=windows_64
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=c:\jre
set JAVA=%JAVA_HOME%\bin\reqenvdeployproc.exe
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\reqenvdeployproc-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore Immediate Deployment Processing Requestor once installed as a Windows service
    echo     %0 restart    restart Xstore Immediate Deployment Processing Requestor once installed as a Windows service
    echo     %0 stop       stop Xstore Immediate Deployment Processing Requestor once installed as a Windows service
    echo     %0 install    install Xstore Immediate Deployment Processing Requestor as a Windows service
    echo     %0 remove     uninstall Xstore Immediate Deployment Processing Requestor as a Windows service
...
:console
SET my_title=Xstore Immediate Deployment Processing Requestor Console (%RANDOM%)
"%_WRAPPER_EXE%" -c %_WRAPPER_CONF% wrapper.java.command=%JAVA%
goto :eof

:start
"%_WRAPPER_EXE%" -t %_WRAPPER_CONF% wrapper.java.command=%JAVA%
goto :eof

:stop
"%_WRAPPER_EXE%" -p %_WRAPPER_CONF% wrapper.java.command=%JAVA%
goto :eof

:install
"%_WRAPPER_EXE%" -i %_WRAPPER_CONF% wrapper.java.command=%JAVA%
goto :eof

:remove
"%_WRAPPER_EXE%" -r %_WRAPPER_CONF% wrapper.java.command=%JAVA%
goto :eof
