...
set _REALPATH=%~dp0
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\checkdigittester.conf"
set _WRAPPED_APP=checkdigittester
set PLATFORM=windows_64
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=c:\jre
set JAVA=%JAVA_HOME%\bin\checkdigittester.exe
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\checkdigittester-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore Check Digit Tester once installed as a Windows service
    echo     %0 restart    restart Xstore Check Digit Tester once installed as a Windows service
    echo     %0 stop       stop Xstore Check Digit Tester once installed as a Windows service
    echo     %0 install    install Xstore Check Digit Tester as a Windows service
    echo     %0 remove     uninstall Xstore Check Digit Tester as a Windows service
...
:console
SET my_title=Xstore Check Digit Tester Console (%RANDOM%)
