...
set _REALPATH=%~dp0
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\xfg-loganalyzer.conf"
set _WRAPPED_APP=xfg-loganalyzer
set PLATFORM=windows_64
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=c:\jre
set JAVA=%JAVA_HOME%\bin\xfg-loganalyzer.exe
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\xfg-loganalyzer-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore Grocery Client Log Analyzer once installed as a Windows service
    echo     %0 restart    restart Xstore Grocery Client Log Analyzer once installed as a Windows service
    echo     %0 stop       stop Xstore Grocery Client Log Analyzer once installed as a Windows service
    echo     %0 install    install Xstore Grocery Client Log Analyzer as a Windows service
    echo     %0 remove     uninstall Xstore Grocery Client Log Analyzer as a Windows service
...
:console
SET my_title=Xstore Grocery Client Log Analyzer Console (%RANDOM%)
