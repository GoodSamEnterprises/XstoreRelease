...
set _REALPATH=%~dp0
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\encrypt.conf"
set _WRAPPED_APP=encrypt
set PLATFORM=windows_64
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=c:\jre
set JAVA=%JAVA_HOME%\bin\encrypt.exe
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\encrypt-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore Config String Encrypter once installed as a Windows service
    echo     %0 restart    restart Xstore Config String Encrypter once installed as a Windows service
    echo     %0 stop       stop Xstore Config String Encrypter once installed as a Windows service
    echo     %0 install    install Xstore Config String Encrypter as a Windows service
    echo     %0 remove     uninstall Xstore Config String Encrypter as a Windows service
...
:console
SET my_title=Xstore Config String Encrypter Console (%RANDOM%)
