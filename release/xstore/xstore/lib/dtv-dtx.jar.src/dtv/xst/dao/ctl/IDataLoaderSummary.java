/*    */ package dtv.xst.dao.ctl;
/*    */ import dtv.data2.access.IDataAccessObject;
/*    */ import dtv.data2.access.IDataModel;
/*    */ import dtv.event.EventEnum;
/*    */ import java.util.Date;
/*    */ import java.util.List;
/*    */ 
/*    */ public interface IDataLoaderSummary extends IDataModel, IHasDataProperty<IDataLoaderSummaryProperty> {
/*  9 */   public static final EventEnum SET_ORGANIZATIONID = new EventEnum("set organizationId");
/* 10 */   public static final EventEnum SET_FILENAME = new EventEnum("set fileName");
/* 11 */   public static final EventEnum SET_RUNTIME = new EventEnum("set runTime");
/* 12 */   public static final EventEnum SET_CREATEDATE = new EventEnum("set createDate");
/* 13 */   public static final EventEnum SET_CREATEUSERID = new EventEnum("set createUserId");
/* 14 */   public static final EventEnum SET_UPDATEDATE = new EventEnum("set updateDate");
/* 15 */   public static final EventEnum SET_UPDATEUSERID = new EventEnum("set updateUserId");
/* 16 */   public static final EventEnum SET_SUCCESS = new EventEnum("set success");
/* 17 */   public static final EventEnum SET_SUCCESSFULROWS = new EventEnum("set successfulRows");
/* 18 */   public static final EventEnum SET_FAILEDROWS = new EventEnum("set failedRows");
/* 19 */   public static final EventEnum ADD_PROPERTIES = new EventEnum("add Properties");
/* 20 */   public static final EventEnum REMOVE_PROPERTIES = new EventEnum("remove Properties");
/* 21 */   public static final EventEnum SET_PROPERTIES = new EventEnum("set Properties");
/* 22 */   public static final EventEnum START_TRANSACTION = new EventEnum("start transaction");
/* 23 */   public static final EventEnum ROLLBACK_TRANSACTION = new EventEnum("rollback transaction");
/* 24 */   public static final EventEnum COMMIT_TRANSACTION = new EventEnum("commit transaction");
/*    */   
/*    */   void setDAO(IDataAccessObject paramIDataAccessObject);
/*    */   
/*    */   long getOrganizationId();
/*    */   
/*    */   void setOrganizationId(long paramLong);
/*    */   
/*    */   String getFileName();
/*    */   
/*    */   void setFileName(String paramString);
/*    */   
/*    */   long getRunTime();
/*    */   
/*    */   void setRunTime(long paramLong);
/*    */   
/*    */   Date getCreateDate();
/*    */   
/*    */   void setCreateDate(Date paramDate);
/*    */   
/*    */   String getCreateUserId();
/*    */   
/*    */   void setCreateUserId(String paramString);
/*    */   
/*    */   Date getUpdateDate();
/*    */   
/*    */   void setUpdateDate(Date paramDate);
/*    */   
/*    */   String getUpdateUserId();
/*    */   
/*    */   void setUpdateUserId(String paramString);
/*    */   
/*    */   boolean getSuccess();
/*    */   
/*    */   void setSuccess(boolean paramBoolean);
/*    */   
/*    */   int getSuccessfulRows();
/*    */   
/*    */   void setSuccessfulRows(int paramInt);
/*    */   
/*    */   int getFailedRows();
/*    */   
/*    */   void setFailedRows(int paramInt);
/*    */   
/*    */   IDataModel getDataLoaderSummaryExt();
/*    */   
/*    */   void setDataLoaderSummaryExt(IDataModel paramIDataModel);
/*    */   
/*    */   List<IDataLoaderSummaryProperty> getProperties();
/*    */   
/*    */   void setProperties(List<IDataLoaderSummaryProperty> paramList);
/*    */   
/*    */   void addDataLoaderSummaryProperty(IDataLoaderSummaryProperty paramIDataLoaderSummaryProperty);
/*    */   
/*    */   void removeDataLoaderSummaryProperty(IDataLoaderSummaryProperty paramIDataLoaderSummaryProperty);
/*    */   
/*    */   void startTransaction();
/*    */   
/*    */   void rollbackChanges();
/*    */   
/*    */   void commitTransaction();
/*    */ }


/* Location:              C:\WIP-Xstore_Delta-main\release\xstore\xstore\lib\dtv-dtx.jar!\dtv\xst\dao\ctl\IDataLoaderSummary.class
 * Java compiler version: 8 (52.0)
 * JD-Core Version:       1.1.3
 */