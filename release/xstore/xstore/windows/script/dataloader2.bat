...
set _REALPATH=%~dp0
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\dataloader2.conf"
set _WRAPPED_APP=dataloader2
set PLATFORM=windows
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=c:\jre
set JAVA=%JAVA_HOME%\bin\dataloader2.exe
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\dataloader2-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore Dataloader once installed as a Windows service
    echo     %0 restart    restart Xstore Dataloader once installed as a Windows service
    echo     %0 stop       stop Xstore Dataloader once installed as a Windows service
    echo     %0 install    install Xstore Dataloader as a Windows service
    echo     %0 remove     uninstall Xstore Dataloader as a Windows service
...
:console
SET my_title=Xstore Dataloader Console (%RANDOM%)
