...
set _REALPATH=%~dp0
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\datapurger.conf"
set _WRAPPED_APP=datapurger
set PLATFORM=windows
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=c:\jre
set JAVA=%JAVA_HOME%\bin\datapurger.exe
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\datapurger-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore Datapurger once installed as a Windows service
    echo     %0 restart    restart Xstore Datapurger once installed as a Windows service
    echo     %0 stop       stop Xstore Datapurger once installed as a Windows service
    echo     %0 install    install Xstore Datapurger as a Windows service
    echo     %0 remove     uninstall Xstore Datapurger as a Windows service
...
:console
SET my_title=Xstore Datapurger Console (%RANDOM%)
