...
set _REALPATH=%~dp0
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\tlog.conf"
set _WRAPPED_APP=tlog
set PLATFORM=windows
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=c:\jre
set JAVA=%JAVA_HOME%\bin\tlog.exe
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\tlog-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore Tlog Generation once installed as a Windows service
    echo     %0 restart    restart Xstore Tlog Generation once installed as a Windows service
    echo     %0 stop       stop Xstore Tlog Generation once installed as a Windows service
    echo     %0 install    install Xstore Tlog Generation as a Windows service
    echo     %0 remove     uninstall Xstore Tlog Generation as a Windows service
...
:console
SET my_title=Xstore Tlog Generation Console (%RANDOM%)
