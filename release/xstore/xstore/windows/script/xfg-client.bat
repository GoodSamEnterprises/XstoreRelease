...
set _REALPATH=%~dp0
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\xfg-client.conf"
set _WRAPPED_APP=xfg-client
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\xfg-client-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore Grocery Client POS once installed as a Windows service
    echo     %0 restart    restart Xstore Grocery Client POS once installed as a Windows service
    echo     %0 stop       stop Xstore Grocery Client POS once installed as a Windows service
    echo     %0 install    install Xstore Grocery Client POS as a Windows service
    echo     %0 remove     uninstall Xstore Grocery Client POS as a Windows service
...
:console
SET my_title=Xstore Grocery Client POS Console (%RANDOM%)
