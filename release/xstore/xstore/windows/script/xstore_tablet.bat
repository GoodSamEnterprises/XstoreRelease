...
set _REALPATH=%~dp0
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\xstore_tablet.conf"
set _WRAPPED_APP=xstore_tablet
set PLATFORM=windows
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=c:\jre
set JAVA=%JAVA_HOME%\bin\xstore_tablet.exe
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\xstore_tablet-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore Tablet POS once installed as a Windows service
    echo     %0 restart    restart Xstore Tablet POS once installed as a Windows service
    echo     %0 stop       stop Xstore Tablet POS once installed as a Windows service
    echo     %0 install    install Xstore Tablet POS as a Windows service
    echo     %0 remove     uninstall Xstore Tablet POS as a Windows service
...
:console
SET my_title=Xstore Tablet POS Console (%RANDOM%)
