...
set _REALPATH=%~dp0
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\dataloader2-trickle.conf"
set _WRAPPED_APP=dataloader2-trickle
set PLATFORM=windows
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=c:\jre
set JAVA=%JAVA_HOME%\bin\dataloader2-trickle.exe
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\dataloader2-trickle-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore Trickle Dataloader once installed as a Windows service
    echo     %0 restart    restart Xstore Trickle Dataloader once installed as a Windows service
    echo     %0 stop       stop Xstore Trickle Dataloader once installed as a Windows service
    echo     %0 install    install Xstore Trickle Dataloader as a Windows service
    echo     %0 remove     uninstall Xstore Trickle Dataloader as a Windows service
...
:console
SET my_title=Xstore Trickle Dataloader Console (%RANDOM%)
