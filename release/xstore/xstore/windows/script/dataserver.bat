...
set _REALPATH=%~dp0
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\dataserver.conf"
set _WRAPPED_APP=dataserver
set PLATFORM=windows
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=c:\jre
set JAVA=%JAVA_HOME%\bin\dataserver.exe
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\dataserver-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore Dataserver once installed as a Windows service
    echo     %0 restart    restart Xstore Dataserver once installed as a Windows service
    echo     %0 stop       stop Xstore Dataserver once installed as a Windows service
    echo     %0 install    install Xstore Dataserver as a Windows service
    echo     %0 remove     uninstall Xstore Dataserver as a Windows service
...
:console
SET my_title=Xstore Dataserver Console (%RANDOM%)
