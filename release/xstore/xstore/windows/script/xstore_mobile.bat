...
set _REALPATH=%~dp0
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\xstore_mobile.conf"
set _WRAPPED_APP=xstore_mobile
set PLATFORM=windows
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=c:\jre
set JAVA=%JAVA_HOME%\bin\xstore_mobile.exe
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\xstore_mobile-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore Mobile POS once installed as a Windows service
    echo     %0 restart    restart Xstore Mobile POS once installed as a Windows service
    echo     %0 stop       stop Xstore Mobile POS once installed as a Windows service
    echo     %0 install    install Xstore Mobile POS as a Windows service
    echo     %0 remove     uninstall Xstore Mobile POS as a Windows service
...
:console
SET my_title=Xstore Mobile POS Console (%RANDOM%)
