...
set _REALPATH=%~dp0
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\paylog.conf"
set _WRAPPED_APP=paylog
set PLATFORM=windows
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=c:\jre
set JAVA=%JAVA_HOME%\bin\paylog.exe
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\paylog-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore Payroll Log Generator once installed as a Windows service
    echo     %0 restart    restart Xstore Payroll Log Generator once installed as a Windows service
    echo     %0 stop       stop Xstore Payroll Log Generator once installed as a Windows service
    echo     %0 install    install Xstore Payroll Log Generator as a Windows service
    echo     %0 remove     uninstall Xstore Payroll Log Generator as a Windows service
...
:console
SET my_title=Xstore Payroll Log Generator Console (%RANDOM%)
