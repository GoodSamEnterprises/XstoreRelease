...
set _REALPATH=%~dp0
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\transreplicate.conf"
set _WRAPPED_APP=transreplicate
set PLATFORM=windows
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=c:\jre
set JAVA=%JAVA_HOME%\bin\transreplicate.exe
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\transreplicate-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore Transaction Replicator once installed as a Windows service
    echo     %0 restart    restart Xstore Transaction Replicator once installed as a Windows service
    echo     %0 stop       stop Xstore Transaction Replicator once installed as a Windows service
    echo     %0 install    install Xstore Transaction Replicator as a Windows service
    echo     %0 remove     uninstall Xstore Transaction Replicator as a Windows service
...
:console
SET my_title=Xstore Transaction Replicator Console (%RANDOM%)
