...
set _REALPATH=%~dp0
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\hardware-tester.conf"
set _WRAPPED_APP=hardware-tester
set PLATFORM=windows
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=c:\jre
set JAVA=%JAVA_HOME%\bin\hardware-tester.exe
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\hardware-tester-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore Hardware Tester once installed as a Windows service
    echo     %0 restart    restart Xstore Hardware Tester once installed as a Windows service
    echo     %0 stop       stop Xstore Hardware Tester once installed as a Windows service
    echo     %0 install    install Xstore Hardware Tester as a Windows service
    echo     %0 remove     uninstall Xstore Hardware Tester as a Windows service
...
:console
SET my_title=Xstore Hardware Tester Console (%RANDOM%)
