...
set _REALPATH=%~dp0
set _WRAPPER_CONF="%_REALPATH%wrapper\conf\xstore_debug.conf"
set _WRAPPED_APP=xstore_debug
set PLATFORM=windows
set _PLATFORMPATH=%_REALPATH%%PLATFORM%\
set JAVA_HOME=c:\jre
set JAVA=%JAVA_HOME%\bin\xstore_debug.exe
...
set _WRAPPER_EXE=%_PLATFORMPATH%bin\xstore_debug-wrapper.exe
...
if "%COMMAND%" == "" (
    ...
    echo     %0 start      start Xstore POS with Open Debug Port once installed as a Windows service
    echo     %0 restart    restart Xstore POS with Open Debug Port once installed as a Windows service
    echo     %0 stop       stop Xstore POS with Open Debug Port once installed as a Windows service
    echo     %0 install    install Xstore POS with Open Debug Port as a Windows service
    echo     %0 remove     uninstall Xstore POS with Open Debug Port as a Windows service
...
:console
SET my_title=Xstore POS with Open Debug Port Console (%RANDOM%)
