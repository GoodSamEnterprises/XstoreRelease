...
CREATE TABLE upgrade_issues (
    ...
    dup_count	 int,
    pk_conflict nvarchar(max),
    org_code	 nvarchar(30),
    org_value	 nvarchar(60),
    rtl_loc_id	 int
)
else
...
CREATE PROCEDURE validate_orgnode_from_pk (
    ...
    @newpk AS nvarchar(255),
    @has_loc AS bit = 0)
AS
BEGIN
declare @sql nvarchar(max),
	   @col1 nvarchar(30), @val1 varchar(max),
	   @col2 nvarchar(30), @val2 varchar(max),
	   @col3 nvarchar(30), @val3 varchar(max),
	   @col4 nvarchar(30), @val4 varchar(max),
	   @col5 nvarchar(30), @val5 varchar(max),
	   @col6 nvarchar(30), @val6 varchar(max),
	   @col7 nvarchar(30), @val7 varchar(max),
	   @col8 nvarchar(30), @val8 varchar(max),
	   @col9 nvarchar(30), @val9 varchar(max),
	   @colcnt int=0, @crit1 nvarchar(max),@crit2 varchar(max);
...
set @crit1 = @col1 + '='' + cast(i.' + @col1 + ' as nvarchar(max)) + '' and ' + @col2 + '=''''''+ cast(i.' + @col2 + ' as varchar(max)) + ''''''';
if @colcnt>2 set @crit1 = @crit1 + ' and ' + @col3 + '='''''' + cast(i.' + @col3 + ' as nvarchar(max)) + ''''''';
if @colcnt>3 set @crit1 = @crit1 + ' and ' + @col4 + '='''''' + cast(i.' + @col4 + ' as nvarchar(max)) + ''''''';
if @colcnt>4 set @crit1 = @crit1 + ' and ' + @col5 + '='''''' + cast(i.' + @col5 + ' as nvarchar(max)) + ''''''';
if @colcnt>5 set @crit1 = @crit1 + ' and ' + @col6 + '='''''' + cast(i.' + @col6 + ' as nvarchar(max)) + ''''''';
if @colcnt>6 set @crit1 = @crit1 + ' and ' + @col7 + '='''''' + cast(i.' + @col7 + ' as nvarchar(max)) + ''''''';
if @colcnt>7 set @crit1 = @crit1 + ' and ' + @col8 + '='''''' + cast(i.' + @col8 + ' as nvarchar(max)) + ''''''';
if @colcnt>8 set @crit1 = @crit1 + ' and ' + @col9 + '='''''' + cast(i.' + @col9 + ' as nvarchar(max)) + ''''''';
...
exec validate_orgnode_from_pk 'itm_item_restriction','organization_id, restriction_category, restriction_code, effective_date';

exec validate_orgnode_from_pk 'itm_item_restriction_calendar','organization_id, restriction_category, restriction_code, effective_date, day_code, start_time';
