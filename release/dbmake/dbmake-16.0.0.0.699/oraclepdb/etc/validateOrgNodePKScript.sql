...
INSERT INTO DTV.upgrade_issues (table_name,dup_count,pk_conflict,org_code,org_value)
SELECT 'itm_item_restriction', cnt,  'organization_id=' || i.organization_id || ' and restriction_category=''' || i.restriction_category ||''' and restriction_code=''' || i.restriction_code ||''' and effective_date=''' || i.effective_date ||'''', org_code,org_value
from itm_item_restriction i
inner join(select organization_id, restriction_category, restriction_code, effective_date,COUNT(*) cnt from dtv.itm_item_restriction group by organization_id, restriction_category, restriction_code, effective_date having count(*) > 1) a on i.organization_id= a.organization_id and i.restriction_category= a.restriction_category and i.restriction_code= a.restriction_code and i.effective_date= a.effective_date;
 
INSERT INTO DTV.upgrade_issues (table_name,dup_count,pk_conflict,org_code,org_value)
SELECT 'itm_item_restriction_calendar', cnt,  'organization_id=' || i.organization_id || ' and restriction_category=''' || i.restriction_category ||''' and restriction_code=''' || i.restriction_code ||''' and effective_date=''' || i.effective_date ||''' and day_code=''' || i.day_code ||''' and start_time=''' || i.start_time ||'''', org_code,org_value
from itm_item_restriction_calendar i
inner join(select organization_id, restriction_category, restriction_code, effective_date, day_code, start_time,COUNT(*) cnt from dtv.itm_item_restriction_calendar group by organization_id, restriction_category, restriction_code, effective_date, day_code, start_time having count(*) > 1) a on i.organization_id= a.organization_id and i.restriction_category= a.restriction_category and i.restriction_code= a.restriction_code and i.effective_date= a.effective_date and i.day_code= a.day_code and i.start_time= a.start_time;
