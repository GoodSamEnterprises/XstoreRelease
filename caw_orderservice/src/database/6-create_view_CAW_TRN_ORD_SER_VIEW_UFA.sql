  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW "DTV"."CAW_TRN_ORD_SER_VIEW" ("RETRY_COUNT", "UPDATE_DATE", "TRANS_SEQ", "RTL_LOC_ID", "WKSTN_ID", "BUSINESS_DATE", "BEGIN_DATETIME", "TOTAL", "OPERATOR_PARTY_ID", "TRANS_TYPCODE", "EMPLOYEE_ID", "EMP_FULL_NAME", "EMP_PARTY_ID", "STORE_NAME", "LOC_ADDRESS1", "LOC_ADDRESS2", "LOC_CITY", "LOC_STATE", "LOC_POSTAL_CODE", "LOC_COUNTRY", "PARTY_ID", "ALTERNATE_ID", "ORGANIZATION_TYPCODE", "SALUTATION", "FIRST_NAME", "MIDDLE_NAME", "LAST_NAME", "SUFFIX", "ORGANIZATION_NAME", "ADDRESS_TYPE", "ADDRESS1", "ADDRESS2", "ADDRESS3", "ADDRESS4", "CITY", "STATE", "POSTAL_CODE", "COUNTRY", "COUNTY") AS 
  SELECT COS.RETRY_COUNT,COS.UPDATE_DATE,TRN.TRANS_SEQ, TRN.RTL_LOC_ID, TRN.WKSTN_ID, TRN.BUSINESS_DATE,TRN.BEGIN_DATETIME, TRN.TOTAL, TRN.OPERATOR_PARTY_ID,TRN.TRANS_TYPCODE,
        HRS.LOGIN_ID AS EMPLOYEE_ID, CONCAT(CONCAT(EMP.FIRST_NAME, ' '), EMP.LAST_NAME) AS EMP_FULL_NAME, EMP.PARTY_ID EMP_PARTY_ID,LOC.STORE_NAME, LOC.ADDRESS1 LOC_ADDRESS1, LOC.ADDRESS2 LOC_ADDRESS2, LOC.CITY LOC_CITY, LOC.STATE LOC_STATE, LOC.POSTAL_CODE LOC_POSTAL_CODE, LOC.COUNTRY LOC_COUNTRY,
...
FROM CAW_TRN_ORD_SER COS 
  ...
  LEFT JOIN (CRM_PARTY CRM 
      ...
      JOIN CRM_PARTY_LOCALE_INFORMATION PLI ON CRM.PARTY_ID = PLI.PARTY_ID
 ...
 LEFT JOIN TSN_TNDR_CONTROL_TRANS TTCT ON COS.TRANS_SEQ = TTCT.TRANS_SEQ AND
  ...
  LEFT JOIN (CRM_PARTY CRM2 
      ...
      JOIN CRM_PARTY_LOCALE_INFORMATION PLI2 ON CRM2.PARTY_ID = PLI2.PARTY_ID
